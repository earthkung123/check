            ...
            <div class="col-sm-12 col-lg-12 ">
             ...
             <script type="text/javascript">window.onload = date_time('date_time');</script>
                ...
                <tbody>
                  ...
                  $sql ="SELECT * FROM student_of_subject WHERE subject_id = '$subject_id' ";
                  $Query= mysqli_query($connect,$sql);
                   $i = 1;
                  while($arr = mysqli_fetch_array($Query)){
     $sql1 ="SELECT * FROM student WHERE Student_ID = ".$arr['student_id']." ";
                  $Query1= mysqli_query($connect,$sql1);
                  while($arr1 = mysqli_fetch_array($Query1)){
                  ?>
                  <tr valign='top'>
                     <form action="check.php?group_id=<?php echo $group_id ?>&subject_id=<?php echo $subject_id ?>" method="POST" >
                     <td align='center'><?php echo $i;  ?></td>
                    <td align='center'><?php echo $arr1['Student_NameTH']; ?></td>
                    <input type="hidden" name="no[]" value="<?php echo $arr['no']; ?>">
                    <input type="hidden" name="student_id[]" value="<?php echo $arr1['Student_ID']; ?>">
              <td align="center"><input  type="radio" name="check[<?php echo $i ?>]" value="0"></td>
              <td align="center"><input  type="radio" name="check[<?php echo $i ?>]" value="1"></td>
              <td align="center"><input  type="radio" name="check[<?php echo $i ?>]" value="2"></td>
              <td align="center"><input  type="radio" name="check[<?php echo $i ?>]" value="3"></td>
              <td align="center"> <input type="text" name="comment[<?php echo $i ?>]"> </td>
                  </tr>
                  <?php
                   }
                  $i++;}
                ...
                <input type="submit" class="btn-success btn-lg" onClick="return confirm('คุณต้องการที่จะบันทึกข้อมูลใช่หรือไม่ ?')">
              </div>
              </form>
              </div>
